...
use moray_cli::config::schema::McpServerConfig;
...
async fn main() -> Result<()> {
    ...
    let (enabled, servers) = match std::fs::read_to_string("config.toml") {
        ...
        Err(_) => {
            let config = moray_cli::Config::load_or_init().await?;
    ...
    let registry = moray_cli::tools::McpRegistry::connect_all(&servers).await?;
