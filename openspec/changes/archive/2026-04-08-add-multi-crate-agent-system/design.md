...
**Crate 命名**：在 **`Cargo.toml`** 中，可发布 crate 使用 **`nova-*` 包名**（如 **`nova-core`**、未来 `nova-session`），降低 crates.io 撞名风险；仓库内目录可采用短名（如 **`core/`**、**`demo/`**），**不必**与包名一致。**demo** 包名建议仍为 **`nova-demo`** 并保持 **`publish = false`**。

**本阶段**：实现 **`nova-core` + demo**；**不实现 `nova-session`**，会话与转录持久化的产品化封装留待后续变更。
...
- 提供 `examples/chat`：**真实 `OpenAICompletionProvider`**（`async-openai` 适配，`nova_core::completions::openai`）由 demo 层 **`completion_provider_from_env`** 读取 **`NOVA_OPENAI_*`** 装配；工具侧现阶段仍可用 demo **mock toolbox**（如 echo）演示授权与事件流。本阶段在 demo 内完成转录加载与 QA 分段（或抽到 `nova-core` 的无依赖小模块），验证回放、未完成段恢复、REPL。
- 集成测试：`nova-core` 使用 **`nova-demo::mock`**（dev-dependency）覆盖恢复、工具授权中断、无 **prefill**（`CompletionCapabilities::prefill_supported`）时「丢弃末轮未确认 completion 并重试」；**不依赖**真实云 API。

**Non-Goals（本阶段）**

- **`nova-session` crate**：转录产品化读写、会话级 API（后续变更）。
...
|--------|------|------|--------|
| `core/` | **`nova-core`** | 类型、状态机、`CompletionProvider`/`Toolbox` trait、ReAct 引擎、`Event`、恢复纯逻辑 | 实现 |
| （预留） | `nova-session` | 转录序列化/反序列化、QA 分段、会话级 API | **不实现** |
| `demo/` | **`nova-demo`**（`publish = false`） | `examples/*`、CLI 薄 UI、集成测试 | 实现 |

依赖方向：**`nova-demo` → `nova-core` only**；**禁止** `nova-core` 依赖 demo。未来引入 `nova-session` 后为 **`nova-demo` → `nova-session` → `nova-core`**。
...
- `nova-core` 为 `Box` / `Arc` 提供对 `CompletionProvider`、`Toolbox` 的 **转发实现**，`Agent::new` 可直接持有上述智能指针。
...
- **OpenAI 兼容的具体实现**：在 **`nova-core` 的 `providers/openai`** 中基于 [`async-openai`](https://crates.io/crates/async-openai) 实现 **`OpenAICompletionProvider`**，映射到 `CompletionProvider`（chunk：`TextDelta`、逐项 **`ToolCall`**、**`Finished`**）；**不在本仓库重复实现**与 OpenAI HTTP/流协议等价的底层客户端。`api_key`、`api_base`、`model` 由调用方显式传入（无 core 内默认模型或 URL）。
...
- 二者返回 **同类异步事件流**。
- 集成方直接消费 `invoke` / `resume` 的事件流（例如 demo 内联 `match`）；若多处需要相同逻辑，再在应用或未来 `nova-session` 层提炼共享驱动，而非当前 core 必选 API。
...
- **本阶段**：分段与文件加载在 **demo**（或 core 内极薄辅助）完成；未来迁移至 `nova-session`。
- **持久化载体（如 `jsonl`）**：初版可采用 **`jsonl` 等简单格式**，**归属 demo 层实现细节**；未来由 **`nova-session` 策略化**（可插拔编码、路径、版本迁移），**不是当前 core 架构的关键决策点**。
...
- **事件持久化格式**：**`jsonl` 初版可接受**；当前为 **demo 层**选型，未来 **`nova-session`** 再策略化；**不列为 core 关键设计前提**。
- **异步运行时**：**统一 `tokio`**。
