> Historical note (2026-04-29): this archived spec uses old names (`OpenAICompletionProvider`, `moray-core` `openai` feature). Current implementation provides OpenAI adapter from `moray-builtin` (`moray_builtin::completions::OpenAIChatCompletion`).

## ADDED Requirements

### Requirement: Non-publishable demo package

The demo workspace package SHALL declare **`publish = false`** in its manifest so it is not published to crates.io; its name MAY be short and repository-local (for example `moray-demo`).
...
The example SHALL use **`moray_core::OpenAICompletionProvider`** (requires **`moray-core`** dependency with feature **`openai`**) for **`CompletionProvider`**, constructed via **`moray_demo::completion_provider_from_env`**, which reads **`MORAY_OPENAI_API_KEY`**, **`MORAY_OPENAI_BASE_URL`**, and **`MORAY_OPENAI_MODEL`**, and SHALL build **`CompletionCapabilities`** from the environment (including optional **`MORAY_OPENAI_PREFILL_SUPPORTED`**) as documented on **`openai_env`**. A demo-level **mock toolbox** (for example echo) MAY be used to illustrate tool listing, authorization, and tool results without requiring a separate tools backend.

In this phase, transcript loading and QA segmentation MAY live in the demo crate or in minimal helpers in `moray-core` that do not constitute the future `moray-session` product API.

#### Scenario: Example documents required environment variables

- **WHEN** reading the `chat` example module documentation (or **`moray_demo::openai_env`**)
- **THEN** it MUST list **`MORAY_OPENAI_API_KEY`**, **`MORAY_OPENAI_BASE_URL`**, and **`MORAY_OPENAI_MODEL`** as required for the live chat client, and document optional capability-related variables (for example **`MORAY_OPENAI_PREFILL_SUPPORTED`**) as applicable

#### Scenario: Workspace tests stay offline

- **WHEN** CI runs `cargo test` for the workspace
- **THEN** tests MUST pass without network access or live model APIs (mock **`CompletionProvider`** / toolbox in **`moray-demo`** or test-only doubles)
...
The workspace SHALL include integration tests (in **`moray-core`** with dev-dependency on **`moray-demo`** and/or in the demo crate as appropriate) that verify core behaviors using mock **`CompletionProvider`** and **`Toolbox`**, including recovery after authorization suspension and the non-prefill partial-turn discard policy.
