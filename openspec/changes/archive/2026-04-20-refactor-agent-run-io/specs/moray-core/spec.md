...
`moray-core` SHALL implement a ReAct-style agent loop as an explicit state machine driven by user input, chat completion stream chunks, toolbox results, and authorization decisions, emitting an ordered sequence of **`AgentRunResponseMessage`** on the **`Agent::run`** output stream.
