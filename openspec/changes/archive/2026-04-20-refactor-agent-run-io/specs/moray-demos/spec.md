...
The workspace SHALL include integration tests under **`demo/tests/*.rs`** using **`demo::mock`** (**`ChatCompletion`** + **`Toolbox`** doubles) against **`moray_core`**. Tests SHALL cover recovery after authorization suspension and replay behavior for **`!prefill_supported`** (including **`replay`** / **`agent`** integration as implemented). **`moray-core`** SHALL NOT depend on **`demo`**.
