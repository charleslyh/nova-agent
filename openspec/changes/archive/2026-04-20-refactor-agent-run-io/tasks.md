...
- [x] 1.1 Add `AgentRunResponseMessage` in `nova-core` with the agreed variants (`InvokeStarted` without user payload); re-export from `lib.rs` as needed.
...
- [x] 3.1 Update `demo` (`chat_view`, examples, `transcript`) to consume `AgentRunResponseMessage`; centralize `run`/`resume` handling per `nova-demos` spec.
- [x] 3.2 Define JSONL / transcript migration or version field if on-disk format changes; document for callers.

## 4. Validation

- [x] 4.1 Update `demo/tests/*.rs` and `nova-core` unit tests for new streams.
- [x] 4.2 `cargo test -p nova-core` and `cargo test -p demo` (and `--features openai` if CI includes it).
