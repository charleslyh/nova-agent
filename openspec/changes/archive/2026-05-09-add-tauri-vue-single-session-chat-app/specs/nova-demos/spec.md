...
Transcript loading and persistence adapters SHALL be provided by the **`nova-builtin`** package (specifically `nova_builtin::stores::JsonlTranscriptStore`) and consumed by **`demo`**; **`demo`** MUST NOT maintain a local copy of `JsonlTranscriptStore`. Session lifecycle orchestration (bootstrap/run/resume/reset/post) SHALL continue to be delegated to **`nova_core::Session`**. The chat view rendering path MUST consume state delta events from a store subscriber, and MUST NOT directly render from the stream returned by **`session.post`** or other single operation calls.
...
#### Scenario: Demo consumes the JSONL transcript store from nova-builtin

- **WHEN** inspecting the demo crate's source and dependencies
- **THEN** the **`chat`** example MUST construct its `JsonlTranscriptStore` from `nova_builtin::stores::JsonlTranscriptStore`
