...
- **`~/.nova/server.toml`**：`[[completions]]` 与 `[[agents]]`；每条记录唯一 **tinyid** `id`（8 位 `[a-z0-9]`）。敏感 `api_key` 可省略（默认等价 `env:NOVA_OPENAI_API_KEY`）或写 `env:<NAME>`；对省略与 `env:` 在**每次构造 completion 适配器（按 turn）** 读环境变量，不在仅加载配置时固定快照。
...
- `server`：`~/.nova/server.toml` 为 **`[[completions]]` / `[[agents]]`**，并有独立 session 配置文件（**`[sessions].default`** 与可选扁平行）；server 按 session 解析 agent 并驱动后续对话。
- `web`: 增加 agents 设置入口、agents card 列表、agent 详情配置卡，以及 composer 中的 Agent 选择菜单。

## Impact

- **配置与数据**：`~/.nova/server.toml` schema 需要替换为 `completions`、`agents` 或等价结构；独立 session 配置文件在 **`[sessions]`** 下保存 **`default`** 与可选的 **`session_id = tinyid`** 行；若某行指向已删除的 agent，加载时 **repair** 为 `default` 并写回；统一 config 类型负责跨文件加载、校验和更新。
...
- **边界约束**：`desktop/client` 的 Tauri command 面仍只承担系统/生命周期职责；`nova-core` 的 `Agent` / `Session` trait-object 边界不因本变更扩大。
