...
- **BREAKING**：移除通过 `ServerOptions` / `resolve()` 为 completion 提供凭据的现有方式；不再从配置或 `MORAY_DESKTOP_TRANSCRIPT_PATH` 读取 transcript 路径。
...
- Transcript 文件路径从配置中删除，改为在 `desktop/server` 内硬编码（沿用或等价于当前 `default_transcript_path()` 的语义：优先 `~/.moray/desktop-transcript.jsonl`，否则临时目录）。
- **配置文件路径由 `desktop/server` 约定**，放在 **`~/.moray/`**（与同目录 transcript、日志等一致）；**仅保留无参 `start()`**：内部完成读取 TOML → 解析敏感引用 → 构造运行时对象 → 启动 HTTP（不再提供单独的「仅 resolved 启动」API）。
...
- `server`：更新配置与凭据解析需求——由 TOML 驱动的强类型 config、**无参 `start()`** 内部完成加载与启动（与旧「先 resolved 再起动」方案合并为同一入口）、`ServerHarness` 依赖 config、transcript 路径硬编码、API key 的 `env:` 引用语义；并 **BREAKING** 移除用于 completion 的旧 `ServerOptions` / `MORAY_OPENAI_*` 回退组合（以任务与设计为准）。

## Impact

- **代码**：`desktop/server/src/lib.rs`（`config` 模块、无参 **`start()`**、`ServerHarness`、测试辅助）、可能新增 `desktop/server` 下 config 子模块与示例 TOML；`desktop/client` 改为仅调用 **`start().await`**（内部已加载默认 `~/.moray/` 下固定文件名），一般不再自行拼路径或先 `load_config`。
