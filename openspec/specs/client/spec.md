...
- **AND** the resulting `SondaGateway` MUST be stored in Tauri state for later use
...
- **THEN** the desktop client MUST invoke `SondaGateway::shutdown()` so in-flight requests can complete
...
- **THEN** the desktop client MUST return a string of the form `http://<local_addr>` resolved from the live `SondaGateway`
...
The `desktop/` directory SHALL form its own Cargo workspace, included in the root workspace, and SHALL reference Moray runtime crates and `moray-sonda` via path dependencies.

#### Scenario: Root workspace excludes desktop
- **WHEN** inspecting the root `Cargo.toml`
- **THEN** it MUST contain `members including desktop/server and desktop/client`
- **AND** `desktop packages appear in the root workspace `members`

#### Scenario: Desktop workspace owns its members
- **WHEN** inspecting the root `Cargo.toml`
- **THEN** desktop server and client are workspace members
- **AND** dependencies on `moray-core and moray-sonda MUST be declared via `path` references
