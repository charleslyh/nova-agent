# moray-core Specification

## Purpose

Defines the **`moray-core`** crate: OpenAI-style **chat completion** trait, **toolbox** trait, ReAct **agent** with recoverable **domain events**, and **replay** helpers used by **`Agent::run`** / **`Session::resume`** (and **`Session::run`** for new user turns).
## Requirements
### Requirement: Multi-crate workspace layout for moray agent runtime

The repository SHALL provide a Rust workspace whose members include **`moray-core`**, **`moray-session`**, **`moray-extensions`**, **`moray-sonda`**, and application crates under **`desktop/`**. **`moray-core` MUST NOT** depend on application or integration crates (`moray-sonda`, `moray-desktop-*`, etc.).

#### Scenario: Core builds without session

- **WHEN** building the **`moray-core`** crate alone
- **THEN** the build MUST succeed without depending on any future **`moray-session`** crate

### Requirement: Chat completion abstraction for the agent loop

`moray-core` SHALL define the **`ChatCompletion`** trait abstracting **OpenAI chat-completion-compatible** behavior used by the ReAct loop: streaming assistant output and tool invocations shaped so that a single mock can stand in for multiple vendors without changing the loop.
...
- **THEN** both paths MUST be able to reuse the same stream-processing logic so **`run`** and **`resume`** are consumed consistently without separate incompatible match arms
...
### Requirement: OpenAI adapter (builtin package)

The workspace SHALL provide **`OpenAIChatCompletion`** as part of the **`moray-extensions`** package (module **`moray_extensions::completions`**), implementing **`moray_core::ChatCompletion`**.

#### Scenario: Core default build has no HTTP client

- **WHEN** building **`moray-core`** alone
- **THEN** the crate MUST NOT require **`async-openai`** for that build
...
**`ChatCompletionResponseChunk`** SHALL include **`TextDone`**, emitted after the last **`TextBlock`** and before the first **`ToolCall`** when a round contains both. Adapters **MUST** synthesize **`TextDone`** when the vendor stream does not provide an explicit equivalent, at the position required by **`moray-core`** documentation.
...
`moray-core` SHALL document ordering rules (including text-only rounds) so UI layers can flush streamed text without guessing from **`TextBlock`** / **`ToolCall`** heuristics alone.
...
`moray-core` SHALL NOT ship a concrete `ToolCallAuthPolicy` implementation. Consumers (desktop apps, production harnesses, persistence-backed policies, test doubles) MUST provide their own.
